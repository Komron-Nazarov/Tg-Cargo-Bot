...
Telegram-бот на Python для оформления и обработки заявок на доставку. Один код поддерживает два режима развёртывания: Railway через long polling и Render Web Service через webhook.
...
Секреты нельзя добавлять в Git. Для Supabase используется `DB_SSL=require`; для локального PostgreSQL без SSL — `DB_SSL=disable`.
