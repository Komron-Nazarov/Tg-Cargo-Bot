...
## Client ID, Tracking Number и Cargo ID
...
Эти идентификаторы не заменяют друг друга. Cargo ID не зависит от Client ID, трек-номера или Telegram username.
...
После успешной приёмки клиент получает уведомление и может открыть груз через кнопку «🚚 Мои грузы». Консолидация, партии, отправка из Китая и последующие статусы пока не реализованы.
...
handlers/               Telegram-обработчики клиентов, tracking, Cargo и склада
repositories/           SQL-запросы orders, clients, trackings и cargos
